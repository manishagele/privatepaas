...
define paascore::clean ($mode, $target) {
  if $mode == 'refresh' {
    exec{
      ...
      "Remove_log_file":
        path    => '/usr/local/sbin:/usr/local/bin:/usr/sbin:/usr/bin:/sbin:/bin',
        onlyif  => "test -f /var/log/wso2-paas.log",
        command => "rm /var/log/wso2-paas.log";

    }
  }
  elsif $mode == 'new' {
    exec { 
      "Stop_process_and_remove_CARBON_HOME_${name}":
        path    => '/usr/local/sbin:/usr/local/bin:/usr/sbin:/usr/bin:/sbin:/bin:/opt/java/bin/',
    ...
    exec { "Stop_process_remove_CARBON_HOME_and_pack_${name}":
        path    => '/usr/local/sbin:/usr/local/bin:/usr/sbin:/usr/bin:/sbin:/bin:/opt/java/bin/',
        command => "kill -9 `cat ${target}/wso2carbon.pid` ; rm -rf ${target} ; rm -f ${::local_package_dir}/wso2${paascore::service_code}-${paascore::version}.zip";
    }
  }
}
